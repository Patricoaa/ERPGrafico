...
export function Step3_PurchasePayment({ paymentData, setPaymentData, total }: Step3_PurchasePaymentProps) {
    ...
    const isMethodAllowed = (methodId: string) => {
        ...
        switch (methodId) {
            ...
            case 'CHECK':
                return allowedMethods.some(m => m.method_type === 'CHECK') // Assuming CHECK is valid for purchases (issuing checks)
            default:
                return false
        }
    }
    ...
    const filteredAccounts = useMemo(() => {
        return accounts.filter(acc => {
            ...
            // Checks for purchases usually mean we are issuing them, so we need an account that allows checks
    ...
    useEffect(() => {
        ...
        if (methodsForType.length >= 1) {
            ...
            // Check if current account is valid for the new method/type
    ...
    const methods = useMemo(() => {
        const availableMethods = [
            ...
            // For purchases, "Card" usually means we are paying with a company card
            {
                id: 'CARD',
                label: 'Tarjeta',
                icon: CreditCard,
    ...
    }, [accounts, allowedMethods, isMethodAllowed])
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-blue-500/5 rounded-xl border border-blue-500/10 flex justify-between items-center h-24">
                    <div>
                        <Label className="text-[10px] font-bold uppercase text-muted-foreground">Monto a Pagar</Label>
                ...
                {paymentData.amount < total && (
                    <div className="p-4 bg-orange-500/5 rounded-xl border border-orange-500/10 flex justify-between items-center h-24">
                        <div>
                            <Label className="text-[10px] font-bold uppercase text-muted-foreground">Deuda Pendiente</Label>
                ...
                {paymentData.amount > total && (
                    <div className="p-4 bg-emerald-500/5 rounded-xl border border-emerald-500/10 flex justify-between items-center h-24">
                        <div>
                            <Label className="text-[10px] font-bold uppercase text-muted-foreground">Excedente (Vuelto)</Label>
                        ...
                        <div key={m.id} className="relative group h-full">
                            <Label
                                ...
                                onClick={(e) => {
                                    ...
                                    if (paymentData.method === m.id && m.id !== 'TRANSFER' && m.id !== 'CHECK') {
                                        openAmountModal()
                                    }
                                }}
                            >
                                ...
                                {paymentData.method === m.id && (
                                    <div className="mt-2 space-y-3 pt-3 border-t w-full animate-in fade-in slide-in-from-top-2" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        {methodsForType.length > 0 && (
                                            <div className="space-y-1">
                                                <Label className="text-[10px] font-bold uppercase text-muted-foreground">
                                                    {paymentData.method === 'TRANSFER' ? 'Cuenta de Origen' : 'Cuenta de Pago'}
                                                </Label>
                                                <select
            ...
            <Dialog open={isAmountModalOpen} onOpenChange={setIsAmountModalOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Ingrese el monto a pagar para esta compra.
                        </DialogDescription>
                    </DialogHeader>
