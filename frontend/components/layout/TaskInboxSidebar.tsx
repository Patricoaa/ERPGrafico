...
export function TaskInboxSidebar({ isOpen, onClose }: TaskInboxSidebarProps) {
    return (
        <aside
            className={cn(
                "fixed right-0 top-0 w-1/4 min-w-[320px] max-w-[450px] bg-slate-950 border-l border-white/5 h-screen flex flex-col transition-all duration-300 ease-in-out overflow-hidden z-50 shadow-[0_0_50px_rgba(0,0,0,0.5)]",
                isOpen ? "translate-x-0" : "translate-x-full"
            )}
        >
            {/* Header */}
            <div className="flex items-center justify-between p-5 border-b border-white/5 bg-slate-900/40 backdrop-blur-xl">
