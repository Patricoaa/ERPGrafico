...
import { Package, Tags, Scale, DollarSign } from "lucide-react"
            ...
            <Tabs defaultValue="products" className="space-y-4" onValueChange={setActiveTab}>
                <div className="flex justify-center">
                    <TabsList className="grid w-full max-w-xl grid-cols-3 bg-muted/50 rounded-full h-12 p-1 border">
                        ...
                        <TabsTrigger value="pricing-rules" className="rounded-full data-[state=active]:bg-background data-[state=active]:shadow-sm gap-2">
                            <DollarSign className="h-4 w-4" />
                            <span className="max-sm:hidden">Reglas de Precio</span>
                        </TabsTrigger>
                    </TabsList>
                </div>
