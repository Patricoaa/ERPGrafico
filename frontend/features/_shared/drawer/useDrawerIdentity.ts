...
import { getEntityMetadata, getEntityIcon, renderEntitySubtitle, renderEntitySubtitleSuffix, formatEntityDisplay } from "@/lib/entity-registry"
...
export interface UseDrawerIdentityOptions {
  ...
  feminine?: boolean
  /** Subtítulo estático (string o JSX). Anula subtitleTemplate del registry. */
  overrideSubtitle?: ReactNode
...
function formatEntityTitle(label: string, mode: DrawerMode, data?: DrawerIdentityInput, feminine?: boolean) {
  ...
  if (meta?.shortTemplate && record) {
    const display = formatEntityDisplay(label, record)
...
export function useDrawerIdentity(
  ...
  const title = options?.overrideTitle ?? formatEntityTitle(label, mode, data, feminine)

  const baseSubtitle = options?.overrideSubtitle ?? renderEntitySubtitle(label, data as Record<string, unknown> | null | undefined) ?? formatDefaultSubtitle(mode, label, feminine)
  const suffix = options?.subtitleSuffix ?? renderEntitySubtitleSuffix(label, data as Record<string, unknown> | null | undefined)
  const subtitleValue = suffix ? `${baseSubtitle} · ${suffix}` : baseSubtitle
