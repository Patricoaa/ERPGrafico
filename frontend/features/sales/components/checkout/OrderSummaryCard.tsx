            ...
            <div className="p-6 bg-background border-t shadow-[0_-4px_20px_-10px_oklch(0.12_0.02_240_/_0.1)] space-y-3">
                ...
                {lineDiscounts > 0 && (
                    <div className="flex justify-between text-xs font-bold text-primary/70 italic">
                        ...
                        <span className="whitespace-nowrap">-{formatCurrency(lineDiscounts)}</span>
                    </div>
                )}
                {totalDiscountAmount > 0 && (
                    <div className="flex justify-between text-xs font-bold text-primary">
                        <span>Descuento Global</span>
                        <span className="whitespace-nowrap">-{formatCurrency(totalDiscountAmount)}</span>
                    </div>
                )}
                <Separator className="my-2 opacity-50" />
                <div className="flex justify-between items-center pt-1">
                    <span className="text-4xs font-bold uppercase tracking-widest text-muted-foreground/60">Total Venta</span>
                    <span className="text-2xl font-bold text-primary tracking-tighter whitespace-nowrap">
                ...
                {!customer?.credit_blocked && (
                    <>
                        ...
                        <div className="flex justify-between items-center pt-1">
                            ...
                            <span className={`font-bold ${Number(customer?.credit_available) < total ? 'text-destructive' : 'text-success'}`}>
                        ...
                        <div className="space-y-1.5 pt-1">
                            ...
                            {(paymentAmount || 0) !== total && (
                                <div className="flex justify-between text-xs font-bold text-muted-foreground/80">
                                    ...
                                    <span className={cn((paymentAmount || 0) > total ? "text-success" : "text-warning")}>
