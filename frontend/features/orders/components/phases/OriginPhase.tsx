        ...
        >
            <div className="flex flex-col gap-1">
                {(activeDoc?.lines || activeDoc?.items || []).slice(0, 3).map((line: OrderLine, idx: number) => (
                    <div key={idx} className="flex items-start justify-between text-3xs gap-2 py-0.5 border-b border-border last:border-0">
                ...
                {(activeDoc?.lines || activeDoc?.items || []).length > 3 && (
                    <div className="text-4xs text-muted-foreground/60 italic pt-1 flex justify-between items-center border-t border-border/10 mt-1">
                        ...
                        <span className="font-bold text-primary/60 text-4xs uppercase tracking-widest">Total {(activeDoc?.lines || activeDoc?.items || []).length} ítems</span>
                    </div>
                )}
            </div>
        </PhaseCard>
