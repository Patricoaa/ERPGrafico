...
export function CreditUtilizationRing({
    ...
    const billedPct = limit > 0 ? (billed / limit) * 100 : 0
    const unbilledPct = limit > 0 ? (unbilled / limit) * 100 : 0

    const circumference = 2 * Math.PI * 42
    const billedOffset = circumference - (billedPct / 100) * circumference
    const unbilledOffset = circumference - ((billedPct + unbilledPct) / 100) * circumference
        ...
        <div className={cn('flex items-center gap-5', className)}>
            ...
            <div className="space-y-2 min-w-0">
                <div className="flex items-center gap-2 text-[11px]">
                    ...
                    <span className="font-bold tabular-nums ml-auto">{format(billed)}</span>
                </div>
                <div className="flex items-center gap-2 text-[11px]">
