...
import { Calendar, ArrowRight, GitBranch, Receipt, FileBadge, Package } from "lucide-react"
...
export function InvoiceCard({ item, type, onClick, onActionSuccess, className, isSelected = false, visibleColumns, isDetailView = false }: InvoiceCardProps) {
    ...
    return (
        <EntityCard
            isSelected={isSelected && !isDetailView}
            ...
            <EntityCard.Header
                title={
                    <div className="flex items-center gap-3">
                        {/* Document icon */}
                        <div className={cn(
                            "w-10 h-10 rounded-sm flex flex-col items-center justify-center border transition-transform duration-300 group-hover:scale-105 shrink-0",
                ...
                trailing={
                    <div className="flex items-center gap-4">
                        {/* Status + Adjustments — hidden on mobile */}
