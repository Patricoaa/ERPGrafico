...
5. **Movimiento con propósito.** Transiciones locales vía `<FadeIn>`, sin framer-motion directo en páginas, sin animaciones que frenen al operador. Los paneles de borde (CollapsibleSheet/Drawer embebido) usan slide+fade asimétrico GPU-only con scrim sutil sobre el canvas. → [component-animation.md](docs/20-contracts/component-animation.md)
