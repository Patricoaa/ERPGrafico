...
15. Border radius default: `0.5rem`. No `rounded-xl` on form components without ADR. `rounded-full` limited to: `Badge.Hub` (status ring), avatar/entity icons, icon-only buttons, filter pills, skeleton circles, scrollbar thumbs, status dot indicators. Badges/chips use `rounded-sm` (ADR-0068). `rounded-none` only permitted in: nested analytics cards (within a Card container), segmented toolbar buttons, underline tabs, error boundary containers, specific skeleton presets.
