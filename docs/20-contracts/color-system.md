...
| Tab underline + module rail + page navbar (TabBar toolbar) + top navheader (ModuleNavigationMenu) (navigation) | Positional process-ink accents — C→M→Y→K by position (hover/active/open), decorative (ADR-0076) | `text-{ink}` / `bg-{ink}` on `--color-cyan` / `--color-magenta` / `--color-yellow` / `--color-black` |
...
- **Positional Layer 1 navigation accents** (Tab underline, module rail + page navbar/TabBar toolbar + top navheader/ModuleNavigationMenu, C→M→Y→K by position) are authorized by ADR-0076 and documented in §8.
