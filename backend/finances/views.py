...
def get_balance_sheet_data(request):
    ...
    data = cache_report(
        ...
        timeout=90,
        generator=lambda: FinanceService.get_balance_sheet(
            end_date_obj, start_date_obj, comp_end_obj, comp_start_obj
        ),
    )
...
def get_income_statement_data(request):
    ...
    data = cache_report(
        ...
        generator=lambda: FinanceService.get_income_statement(
            to_date(start_date), to_date(end_date), to_date(comp_start), to_date(comp_end)
        ),
    )
    return Response(data)
...
def get_trial_balance_data(request):
    ...
    data = cache_report(
        ...
        params={
            "start": str(start_date_obj) if start_date_obj else None,
            "end": str(end_date_obj) if end_date_obj else None,
        },
        timeout=90,
...
def get_financial_analysis_data(request):
    ...
    data = cache_report(
        ...
        timeout=90,
        generator=lambda start_date_obj=start_date_obj, end_date_obj=end_date_obj: (
            FinanceService.get_financial_analysis(start_date_obj, end_date_obj)
        ),
    )
    return Response(data)
