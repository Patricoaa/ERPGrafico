...
class TestCashFlowIntegrity(TestCase):
    ...
    def test_cash_flow_balanced_operating(self):
        ...
        entry = JournalEntry.objects.create(
            date=date(2024, 1, 15), description="Pago Cliente", status="POSTED"
        )
        JournalItem.objects.create(entry=entry, account=self.t_acc.account, debit=100)
        JournalItem.objects.create(
            entry=entry, account=Account.objects.get(code="1.1.02.01"), credit=100
        )
    ...
    def test_cash_flow_discrepancy_detection(self):
        ...
        entry = JournalEntry.objects.create(
            date=date(2024, 2, 10), description="Movimiento Huérfano", status="POSTED"
        )
    ...
    def test_internal_transfer_neutrality(self):
        ...
        TreasuryAccount.objects.create(
            name="Caja Secundaria", account=acc_2, account_type=TreasuryAccount.Type.CASH
        )
        ...
        entry = JournalEntry.objects.create(
            date=date(2024, 3, 10), description="Traspaso", status="POSTED"
        )
