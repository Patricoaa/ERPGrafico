...
class IndicatorService:
    """
    Lógica de negocio del modelo `IndicatorValue`.

    Permite cargar valores manualmente (CRUD) y consultar con fallback
    "last known good" (delegado al método de clase del modelo).

    El feed automático desde mindicador.cl está pensado como opt-in: la
    red puede no estar disponible y la operatoria debe sobrevivir con
    carga manual. Ver `docs/50-audit/bancos/fase-2-creditos-bancarios.md`
    (F2.1).
    """

    UF_URL = "https://mindicador.cl/api/uf"
    UTM_URL = "https://mindicador.cl/api/utm"
    USD_URL = "https://mindicador.cl/api/dolar"

    @staticmethod
    def upsert(indicator: str, date, value, source: str = "manual", notes: str = ""):
        """Crea o actualiza el valor de un indicador para una fecha."""
    ...
    @staticmethod
    def fetch_from_mindicador(indicator: str, days: int = 30) -> int:
        """
        Intenta descargar valores desde mindicador.cl (API pública, sin auth).
        Retorna la cantidad de valores nuevos/actualizados insertados.

        Si no hay red o el indicador no es UF/UTM/USD, retorna 0 sin error:
        el sistema sigue funcionando con los valores cargados manualmente.
        """
        ...
        url_map = {
            IndicatorValue.Indicator.UF: IndicatorService.UF_URL,
            IndicatorValue.Indicator.UTM: IndicatorService.UTM_URL,
            IndicatorValue.Indicator.USD: IndicatorService.USD_URL,
        }
        url = url_map.get(indicator)
        if not url:
            return 0

        try:
            req = urllib.request.Request(url, headers={'User-Agent': 'ERPGrafico/1.0'})
            with urllib.request.urlopen(req, timeout=5) as resp:
                payload = json.loads(resp.read().decode('utf-8'))
        except Exception:
            return 0  # sin red: no rompe

        inserted = 0
        for serie in payload.get('serie', [])[:days]:
            try:
                fecha = serie['fecha'][:10]  # 'YYYY-MM-DD'
                valor = serie['valor']
                from datetime import date as _date
                d = _date.fromisoformat(fecha)
                IndicatorService.upsert(
                    indicator=indicator,
                    date=d,
                    value=valor,
                    source='mindicador.cl',
                    notes='Feed automático',
                )
                inserted += 1
            except Exception:
                continue
