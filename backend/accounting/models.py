...
class Account(TimeStampedModel):
    code = models.CharField(
        _("Código"), max_length=20, unique=True, blank=True, help_text="Ej: 1.1.01.001"
    )
    ...
    is_active = models.BooleanField(
        ...
        default=True,
        db_index=True,
        help_text=_("Desactivar para archivar la cuenta; los mayores históricos siguen visibles."),
    )
    ...
    is_category = models.CharField(
        _("Categoría Estado Resultados"),
        max_length=30,
        choices=ISCategory.choices,
        null=True,
        blank=True,
    )
    cf_category = models.CharField(
        _("Categoría Flujo de Caja"),
        max_length=30,
        choices=CFCategory.choices,
        null=True,
        blank=True,
    )
    bs_category = models.CharField(
        _("Categoría Balance"), max_length=30, choices=BSCategory.choices, null=True, blank=True
    )
    ...
    def debit_total(self):
        ...
        return sum(
            item.debit
            for item in self.journal_items.filter(
    ...
    def clean(self):
        ...
        if self.parent:
            ...
            if self.account_type != self.parent.account_type:
                raise ValidationError(
                    {
                        "account_type": _(
                            "El tipo de cuenta debe coincidir con el de la cuenta padre (%s)."
                        )
                        % self.parent.get_account_type_display()
                    }
                )
        ...
        if self.is_category and self.account_type not in [AccountType.INCOME, AccountType.EXPENSE]:
            raise ValidationError(
                {
                    "is_category": _(
                        "Solo las cuentas de Ingresos y Gastos pueden tener categorías de Estado de Resultados."
                    )
                }
            )

        if self.bs_category and self.account_type not in [
            AccountType.ASSET,
            ...
            raise ValidationError(
                {
                    "bs_category": _(
                        "Solo las cuentas de Activo, Pasivo y Patrimonio pueden tener categorías de Balance General."
                    )
                }
            )

        if self.bs_category:
            if self.account_type == AccountType.ASSET and self.bs_category not in [
                BSCategory.CURRENT_ASSET,
                BSCategory.INVENTORY,
                BSCategory.NON_CURRENT_ASSET,
            ]:
                raise ValidationError(
                    {
                        "bs_category": _(
                            "Las cuentas de Activo solo pueden ser Activo Corriente, Inventario o No Corriente."
                        )
                    }
                )
            if self.account_type == AccountType.LIABILITY and self.bs_category not in [
                BSCategory.CURRENT_LIABILITY,
                BSCategory.NON_CURRENT_LIABILITY,
            ]:
                raise ValidationError(
                    {
                        "bs_category": _(
                            "Las cuentas de Pasivo solo pueden ser Pasivo Corriente o No Corriente."
                        )
                    }
                )
            if self.account_type == AccountType.EQUITY and self.bs_category != BSCategory.EQUITY:
                raise ValidationError(
                    {
                        "bs_category": _(
                            "Las cuentas de Patrimonio solo pueden ser categoría Patrimonio."
                        )
                    }
                )
        ...
        if settings:
            ...
            if depth > settings.hierarchy_levels:
                raise ValidationError(
                    {
                        "parent": _(
                            "No se puede crear la cuenta. Se ha alcanzado el límite de %d niveles de jerarquía."
                        )
                        % settings.hierarchy_levels
                    }
                )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.code:
            ...
            if self.parent:
                ...
                if self.account_type == AccountType.ASSET:
                    ...
                    prefix = settings.liability_prefix
                elif self.account_type == AccountType.EQUITY:
                    prefix = settings.equity_prefix
                elif self.account_type == AccountType.INCOME:
                    prefix = settings.income_prefix
            ...
            siblings = Account.objects.filter(code__startswith=prefix, parent=self.parent).order_by(
                "-code"
            )
        ...
        if self.code and len(self.code) > 20:
            raise ValidationError(
                {"code": _("El código generado excede el límite de 20 caracteres.")}
            )
...
class JournalEntry(AuditedModel):
    ...
    number = models.CharField(
        _("Número"), max_length=20, unique=True, editable=False, null=True, blank=True
    )
    ...
    is_manual = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "True si fue creado desde el formulario contable. False si fue generado automáticamente (facturación, despacho, etc.)"
        ),
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.period_closed and not is_closing_entry:
            if is_new:
                ...
                raise ValidationError(
                    _(
                        "No se puede registrar un asiento contable en un periodo cerrado. Por favor, verifique la fecha o solicite la reapertura del periodo."
                    )
                )
            else:
                ...
                if self.status != JournalEntry.Status.CANCELLED:
                    raise ValidationError(
                        _(
                            "No se puede modificar un asiento de un periodo cerrado. Solo se permite la anulación."
                        )
                    )
...
class JournalItem(TimeStampedModel):
    ...
    partner_name = models.CharField(
        _("Nombre Socio (Legacy)"),
        max_length=255,
        blank=True,
        help_text=_("Campo legacy para preservar datos históricos de texto."),
    )
    ...
    def clean(self):
        if self.debit > 0 and self.credit > 0:
            raise ValidationError(
                _("Un apunte no puede tener montos en Debe y Haber simultáneamente.")
            )
        if self.account and not self.account.is_selectable:
            raise ValidationError(
                _(
                    "La cuenta %(account)s tiene subcuentas y no es seleccionable para asientos contables."
                ),
...
class AccountingSettings(TimeStampedModel):
    ...
    default_payable_account = models.ForeignKey(
        Account, on_delete=models.SET_NULL, null=True, blank=True, related_name="settings_payable"
    )
    default_revenue_account = models.ForeignKey(
        Account, on_delete=models.SET_NULL, null=True, blank=True, related_name="settings_revenue"
    )
    default_expense_account = models.ForeignKey(
        Account, on_delete=models.SET_NULL, null=True, blank=True, related_name="settings_expense"
    )
    default_tax_receivable_account = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="settings_tax_receivable",
    )
    default_tax_payable_account = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="settings_tax_payable",
    )
    ...
    stock_input_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de Pasivo (ej: Proveedores por Facturar) usada como contrapartida en recepciones."
        ),
    )
    ...
    default_consumable_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto usada por defecto para productos consumibles (tintas, papel, etc.) - ej: 5.2.05"
        ),
    )
    ...
    merchandise_cogs_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta 5.1.01 - Costo de ventas para productos STORABLE comprados para reventa"
        ),
    )
    ...
    manufactured_cogs_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta 5.1.02 - Costo de ventas para productos MANUFACTURABLE fabricados internamente"
        ),
    )
    ...
    default_service_expense_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto usada por defecto para productos de tipo servicio (ej: 5.1.03)."
        ),
    )
    ...
    default_service_revenue_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de ingreso usada por defecto para productos de tipo servicio (ej: 4.1.02)."
        ),
    )
    ...
    default_subscription_expense_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto usada por defecto para productos de tipo suscripción (ej: 5.1.04)."
        ),
    )
    ...
    default_subscription_revenue_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de ingreso usada por defecto para productos de tipo suscripción (ej: 4.1.03)."
        ),
    )
    ...
    adjustment_income_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de ingreso para registrar ganancias de inventario (ej: Otros Ingresos)."
        ),
    )
    ...
    check_portfolio_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de activo (documentos por cobrar) donde se registran los cheques de terceros recibidos hasta su depósito/cobro."
        ),
    )
    issued_checks_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de pasivo donde se registran los cheques propios emitidos hasta su cobro efectivo."
        ),
    )
    ...
    interest_expense_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto para intereses pagados de préstamos bancarios y tarjetas de crédito."
        ),
    )
    ...
    loan_penalty_expense_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto para el interés penal (mora) de cuotas vencidas de préstamos."
        ),
    )
    loan_commission_expense_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto para la comisión de apertura cobrada al desembolso de un préstamo."
        ),
    )
    loan_stamp_tax_expense_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de gasto para el impuesto de timbres y estampillas (ITE) cobrado al desembolso."
        ),
    )
    ...
    pos_default_credit_percentage = models.DecimalField(
        ...
        default=0,
        help_text=_(
            "Porcentaje de la venta que se puede asignar como crédito por defecto si el cliente no tiene línea de crédito."
        ),
    )
    ...
    credit_auto_block_days = models.IntegerField(
        ...
        default=60,
        help_text=_(
            "Días máximos de mora antes de bloquear automáticamente la capacidad de crédito. Dejar en blanco para desactivar el auto-bloqueo."
        ),
    )
    ...
    partner_capital_receivable_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de activo donde se registran las deudas de los socios por capital suscrito no pagado (ej: 1.1.04)."
        ),
    )
    partner_capital_social_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta global de patrimonio para el capital social de la empresa (ej: 3.1.01)."
        ),
    )
    partner_provisional_withdrawal_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta padre de patrimonio (contra) para retiros provisorios de socios (ej: 3.1.05)."
        ),
    )
    ...
    partner_dividends_payable_account = models.ForeignKey(
        ...
        help_text=_(
            "Cuenta de pasivo corriente para dividendos declarados pendientes de pago (ej: 2.1.07)."
        ),
    )
    ...
    hierarchy_levels = models.PositiveSmallIntegerField(
        _("Niveles de Jerarquía"), default=4, help_text=_("Cantidad total de niveles (2-5).")
    )
    code_separator = models.CharField(
        _("Separador"),
        max_length=1,
        default=".",
        help_text=_("Símbolo usado para separar niveles."),
    )
    ...
    @classmethod
    def get_solo(cls):
        from core.cache import CACHE_KEY_ACCOUNTING_SETTINGS, cached_singleton
...
class BudgetItem(TimeStampedModel):
    ...
    amount = models.DecimalField(
        _("Monto Presupuestado"), max_digits=20, decimal_places=0, validators=[MinValueValidator(0)]
    )
            ...
            "tabs": [
                {
                    "id": "main",
                    "label": "General",
                    "fields": ["budget", "account", "year", "month", "amount"],
                }
            ]
        }
