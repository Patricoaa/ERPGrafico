...
class AccountingServiceTest(TestCase):
    ...
    def test_post_balanced_entry(self):
        ...
        JournalItem.objects.create(
            entry=entry, account=self.asset_account, debit=Decimal("100.00"), credit=Decimal("0.00")
        )
        JournalItem.objects.create(
            entry=entry,
    ...
    def test_prevent_unbalanced_entry(self):
        ...
        JournalItem.objects.create(
            entry=entry, account=self.income_account, debit=Decimal("0.00"), credit=Decimal("50.00")
        )  # Missing 50
