...
class BulkImportMixin:
    ...
    def bulk_import(self, request):
        ...
        try:
            ...
            return Response(
                {"message": f"Se importaron {items_created} registros exitosamente."},
...
class AuditHistoryMixin:
    ...
    def history(self, request, pk=None):
        ...
        if not hasattr(instance, "history"):
            return Response(
                {"detail": "History not tracked for this model."},
...
class TotalsCalculationMixin:
    ...
    def recalculate_totals(self, commit=True):
        ...
        if self.totals_strategy is None:
            raise NotImplementedError(f"{self.__class__.__name__} debe definir 'totals_strategy'.")
