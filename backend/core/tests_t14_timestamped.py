...
class TimeStampedModelT14Test(TestCase):
    ...
    def _assert_timestamps(self, instance, label: str):
        self.assertTrue(
            hasattr(instance, "created_at"), f"{label} debe tener created_at (TimeStampedModel)"
        )
        self.assertTrue(
            hasattr(instance, "updated_at"), f"{label} debe tener updated_at (TimeStampedModel)"
        )
        ...
        self.assertLessEqual(
            instance.created_at, instance.updated_at, f"{label}: created_at debe ser <= updated_at"
        )
    ...
    def test_budget_item_has_timestamps(self):
        ...
        item = BudgetItem.objects.create(
            budget=budget, account=account, year=2024, month=1, amount=100000
        )
