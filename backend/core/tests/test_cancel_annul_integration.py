...
from sales.models import SaleDelivery, SaleOrder
from sales.services import SaleOrderService, SalesService
...
def _posted_je(env, suffix="001"):
    ...
    income_acc, _created = Account.objects.get_or_create(
        code="4.1.01.001",
        defaults=dict(name="Ventas Test", account_type=AccountType.INCOME),
    )
    asset_acc = Account.objects.get(code="1.1.01.001")
    je = JournalEntry.objects.create(
        description=f"JE integración {suffix}",
...
class TestPRBAuditAndSemantics:
    ...
    def test_035_annul_without_reason_blocked(self, env):
        ...
        inv = _invoice(
            env,
            purchase_order=_purchase_order(env, status=PurchaseOrder.Status.CONFIRMED),
            contact=env["supplier"],
            number="F-035",
            status=Invoice.Status.POSTED,
        )
