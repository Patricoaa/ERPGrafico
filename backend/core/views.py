...
class MyProfileView(APIView):
    ...
    def get(self, request):
        ...
        if user.contact:
            ...
            employee = (
                Employee.objects.filter(contact=user.contact)
                .select_related("contact", "afp")
                .first()
            )
            if employee:
                ...
                payrolls = (
                    Payroll.objects.filter(employee=employee, status=Payroll.Status.POSTED)
                    .select_related(
                ...
                advances = (
                    ...
                    .select_related("employee", "employee__contact", "payroll")
                    .order_by("-date")
                )
                ...
                # Payroll Payments
                payments = (
                    PayrollPayment.objects.filter(payroll__employee=employee)
                    .select_related("payroll", "payroll__employee", "payroll__employee__contact")
                    .order_by("-date")
                )
...
class MyProfilePayrollPreviewView(APIView):
    ...
    def get(self, request, payroll_id):
        ...
        try:
            payroll = (
                Payroll.objects.select_related("employee", "employee__contact")
        ...
        for model, type_slug, type_label in models_to_track:
            if hasattr(model, "history"):
                ...
                for rec in serialized:
                    ...
                    action_verb = (
                        "creó" if h_type == "+" else "editó" if h_type == "~" else "eliminó"
                    )
                    ...
                    rec["description"] = (
                        f"{action_verb.capitalize()} {type_label.lower()} {obj_name}".strip()
                    )
