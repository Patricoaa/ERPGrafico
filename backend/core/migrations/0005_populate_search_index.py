...
def populate_search_index(apps, schema_editor):
    ...
    try:
        ...
        if not entities:
            logger.warning(
                "No entities found in UniversalRegistry during migration. Skipping population."
            )
            return
        ...
        for label, entity in entities.items():
            ...
            if entity.model._meta.db_table not in existing_tables:
                logger.debug(
                    f"Skipping indexing for {label} as table {entity.model._meta.db_table} does not exist yet."
                )
                continue
