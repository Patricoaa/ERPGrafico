...
def test_task204_metrics_endpoint_response(work_order_factory, warehouse_factory):
    ...
    wo_finished = work_order_factory(
        status=WorkOrder.Status.FINISHED, current_stage=WorkOrder.Stage.FINISHED, warehouse=wh
    )
    ...
    user, _ = User.objects.get_or_create(
        username="testadmin", email="admin@test.com", defaults={"is_superuser": True}
    )
