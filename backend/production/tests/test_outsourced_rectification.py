...
from production.models import WorkOrder, WorkOrderHistory, WorkOrderMaterial
...
def test_task209_outsourced_rectification(work_order_factory):
    ...
    component = Product.objects.create(
        name="Outsourced Service", uom=uom, category=prod_cat, track_inventory=False
    )
    ...
    adjustments = [{"material_id": material.id, "actual_quantity": "12", "actual_unit_price": "55"}]

    WorkOrderService.rectify_production(
        work_order=wo, outsourced_adjustments=adjustments, user=None
    )
