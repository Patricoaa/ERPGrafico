...
class WorkOrderViewSet(viewsets.ModelViewSet, AuditHistoryMixin):
    ...
    def update(self, request, *args, **kwargs):
        """
        Overridden to handle file attachments during update.
        """
        # 1. Standard Update
        ...
        try:
            if request.FILES:
                ...
                # Check for design files (design_file_0, design_file_1, etc.)
                ...
                if approval_file:
                    try:
                        validate_file_size(approval_file)
                        validate_file_extension(approval_file)
                        ...
                        # Update stage_data reference
                        # Refresh object to ensure we have latest stage_data from super().update
                        work_order.refresh_from_db()
                        if not work_order.stage_data: work_order.stage_data = {}
                        work_order.stage_data['approval_attachment'] = approval_file.name
                        work_order.save()
                        
                        # Update response data to reflect changes
                        if 'stage_data' in response.data:
                            response.data['stage_data'] = work_order.stage_data
                    except Exception as e:
                        print(f"Error saving approval file: {e}")
                        
        except Exception as e:
            print(f"Error processing files in update: {e}")
