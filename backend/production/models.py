...
class WorkOrder(models.Model):
    ...
    current_stage = models.CharField(
        _("Etapa Actual"), max_length=30, choices=Stage.choices, default=Stage.MATERIAL_ASSIGNMENT
    )
    ...
    estimated_completion_date = models.DateField(
        _("Fecha Estimada de Finalización"), null=True, blank=True
    )
    ...
    no_materials_required = models.BooleanField(
        _("Sin materiales de stock"),
        default=False,
        help_text="Si se marca, se omite el paso de aprobación de materiales",
    )
        ...
        if not self.number:
            ...
            with transaction.atomic():
                self.number = SequenceService.get_next_number(
                    WorkOrder, year_prefix=settings.use_year_prefix
                )
...
class ProductionConsumption(models.Model):
    ...
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.PROTECT, related_name="production_usages"
    )
    quantity = models.DecimalField(
        _("Cantidad"), max_digits=12, decimal_places=4, validators=[MinValueValidator(0)]
    )
...
class WorkOrderMaterial(models.Model):
    ...
    component = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name="ot_material_usages"
    )
    quantity_planned = models.DecimalField(
        _("Cantidad Planificada"),
        max_digits=12,
        decimal_places=4,
        validators=[MinValueValidator(0)],
    )
    quantity_consumed = models.DecimalField(
        _("Cantidad Consumida"),
        max_digits=12,
        decimal_places=4,
        default=0,
        validators=[MinValueValidator(0)],
...
class WorkOrderHistory(models.Model):
    ...
    work_order = models.ForeignKey(
        WorkOrder, on_delete=models.CASCADE, related_name="stage_history"
    )
...
class BillOfMaterials(models.Model):
    ...
    active = models.BooleanField(
        _("Activo"), default=True, help_text="Solo puede haber un BOM activo por producto"
    )
    ...
    def __str__(self):
        ...
        yield_str = (
            f" ({self.yield_quantity} {self.yield_uom.name if self.yield_uom else ''})".strip()
            if self.yield_quantity != 1
            else ""
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.active:
            BillOfMaterials.objects.filter(product=self.product, active=True).exclude(
                pk=self.pk
            ).update(active=False)
