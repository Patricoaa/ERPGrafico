...
def _validate_phase_data(data: dict, prefix: str = "") -> None:
    ...
    if "design_attachments" in data and not isinstance(data["design_attachments"], list):
        raise ValidationError(
            {f"{prefix}design_attachments": "Debe ser una lista de nombres de archivo."}
        )
    ...
    if "comments" in data:
        ...
        for i, comment in enumerate(comments):
            if not isinstance(comment, dict):
                raise ValidationError(
                    {f"{prefix}comments[{i}]": "Cada comentario debe ser un objeto."}
                )
...
def validate_stage_data(stage_data: dict) -> dict:
    ...
    if "phases" in stage_data:
        ...
        if not isinstance(phases, dict):
            raise ValidationError(
                {"phases": "Debe ser un objeto con claves prepress, press, postpress."}
            )
