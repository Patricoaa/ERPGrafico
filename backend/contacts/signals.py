...
def handle_default_contact_flags(sender, instance, created, **kwargs):
    ...
    if instance.is_default_customer:
        Contact.objects.filter(is_default_customer=True).exclude(pk=instance.pk).update(
            is_default_customer=False
        )

    if instance.is_default_vendor:
        Contact.objects.filter(is_default_vendor=True).exclude(pk=instance.pk).update(
