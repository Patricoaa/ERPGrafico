...
class Contact(models.Model):
    ...
    is_active = models.BooleanField(
        ...
        db_index=True,
        help_text=_(
            "Desactivar para archivar el contacto; los documentos históricos lo siguen referenciando."
        ),
    )
    ...
    credit_limit = models.DecimalField(
        _("Límite de Crédito"), max_digits=14, decimal_places=0, null=True, blank=True
    )
    credit_days = models.IntegerField(_("Días Plazo"), default=30, null=True, blank=True)
    credit_blocked = models.BooleanField(
        _("Crédito Bloqueado"),
        default=False,
        help_text=_("Si se marca, el contacto no podrá usar crédito bajo ninguna circunstancia."),
    )

    # Credit Automation Fields
    credit_auto_blocked = models.BooleanField(
        _("Auto-Bloqueado por Mora"),
        default=False,
        help_text=_(
            "El sistema bloquea temporalmente al cliente si excede los días de mora configurados."
        ),
    )
    credit_risk_level = models.CharField(
        _("Nivel de Riesgo"), max_length=20, choices=RiskLevel.choices, default=RiskLevel.LOW
    )
    credit_last_evaluated = models.DateTimeField(
        _("Última Evaluación Automática"), null=True, blank=True
    )
    ...
    is_partner = models.BooleanField(
        _("Es Socio"),
        default=False,
        help_text=_("Marcar si este contacto es socio/accionista de la empresa."),
    )
