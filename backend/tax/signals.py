...
def mark_invoices_as_closed(sender, instance, **kwargs):
    ...
    if instance.status == TaxPeriod.Status.CLOSED:
        ...
        Invoice.objects.filter(date__gte=start_date, date__lt=end_date).update(
            tax_period_closed=False
        )
...
def mark_journal_entries_as_closed(sender, instance, **kwargs):
    ...
    if instance.status == AccountingPeriod.Status.CLOSED:
        ...
        JournalEntry.objects.filter(
            date__gte=start_date, date__lt=end_date, status=JournalEntry.Status.POSTED
        ...
        JournalEntry.objects.filter(date__gte=start_date, date__lt=end_date).exclude(
        ...
        JournalEntry.objects.filter(
            date__gte=start_date, date__lt=end_date, status=JournalEntry.Status.CLOSED
