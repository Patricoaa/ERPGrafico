...
class PurchasingService:
    ...
    def confirm_purchase(order: PurchaseOrder, user=None):
        ...
        DocumentRegistry.for_instance(order).confirm(order, user=user)
        return order
    ...
    def create_receipt_from_note(
        ...
        receipt = PurchaseReceipt.objects.create(
            ...
            warehouse=warehouse,
            receipt_date=receipt_date,
            delivery_reference="Nota Débito",
        ...
        for item in line_data:
            ...
            if not purchase_line:
                raise ValidationError(
                    f"No se encontró línea de compra válida para el item en la orden {order.reference} (ID Línea: {line_id}, ID Prod: {product_id})"
                )

            if quantity > purchase_line.quantity_pending:
                raise ValidationError(
                    f"Cantidad a recibir ({quantity}) excede la pendiente ({purchase_line.quantity_pending}) para {purchase_line.product.name}"
                )
    ...
    def confirm_receipt(receipt: PurchaseReceipt, subscription_dates=None):
        ...
        has_boleta = any(
            inv.dte_type == Invoice.DTEType.BOLETA for inv in receipt.purchase_order.invoices.all()
        )
        ...
        for line in receipt.lines.all():
                ...
                if line.product.product_type == "SUBSCRIPTION":
                    ...
                    if subscription_dates and str(line.product.id) in subscription_dates:
                        try:
                            start_date = datetime.strptime(
                                subscription_dates[str(line.product.id)], "%Y-%m-%d"
                            ).date()
                    ...
                    subscription_instance, created = Subscription.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            if hasattr(receipt.purchase_order, "currency")
                            else "CLP",
                    ...
                    if created:
                        ...
                        subscription_instance.next_payment_date = (
                            SubscriptionService.calculate_next_payment_date(
                                subscription_instance, from_date=start_date
                            )
                        )
            ...
            if has_boleta:
                ...
                effective_unit_cost = line.unit_cost * (
                    Decimal("1") + (tax_rate / Decimal("100.0"))
                )
    ...
    def _update_product_cost(product, quantity, unit_cost):
        ...
        if current_stock <= 0:
            ...
            if total_qty > 0:
                product.cost_price = ((current_value + new_value) / total_qty).quantize(
                    Decimal("1")
                )
    ...
    def _update_order_receiving_status(order):
        ...
        received_qty = sum(line.quantity_received for line in original_lines)
    ...
    def register_note_from_request(request, order: PurchaseOrder):
        ...
        return PurchasingService.create_note(
            order=order,
    ...
    def create_note(
        ...
        if payable_account.account_type != AccountType.LIABILITY:
            ...
            if (
                ...
                and settings.default_payable_account.account_type == AccountType.LIABILITY
            ):
                payable_account = settings.default_payable_account
            else:
                ...
                raise ValidationError(
                    f"La cuenta asignada ({payable_account.name}) no es de Pasivo. Verifique la configuración del proveedor."
                )
        ...
        if return_items:
            for item in return_items:
                ...
                if not purchase_line:
                    raise ValidationError(
                        f"No se encontró línea de compra para producto ID {product_id}"
                    )
    ...
    def annul_receipt(receipt: PurchaseReceipt, user=None, reason: str = ""):
        ...
        if receipt.journal_entry:
            ...
            JournalEntryService.reverse_entry(
                receipt.journal_entry, description=f"Anulación Recepción {receipt.number}"
            )
    ...
    def purchase_checkout_from_request(request) -> dict:
        ...
        return PurchasingService.purchase_checkout(**params)
    ...
    @staticmethod
    @transaction.atomic
    def _purchase_checkout_internal(
        order_data,
        dte_type,
        document_number="",
        document_date=None,
        ...
        if "id" in order_data:
            ...
            if len(order_data) > 1:
                order_serializer = WritePurchaseOrderSerializer(
                    order, data=order_data, partial=True
                )
        ...
        if payment_method != "CREDIT":
            ...
            if (
                payment_method_inst is not None
                and installments > 1
                and payment_method_inst.method_type == "CREDIT_CARD"
            ):
        ...
        if receipt_type == "IMMEDIATE":
            ...
            if (
                not target_warehouse
                and not order.lines.filter(
                    product__product_type__in=["STORABLE", "CONSUMABLE", "MANUFACTURABLE"]
                ).exists()
            ):
        ...
        return {"order": order, "invoice": invoice, "payment": payment, "receipt": receipt}
