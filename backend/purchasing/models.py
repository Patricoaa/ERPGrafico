...
class PurchaseOrder(TransactionalDocument, TotalsCalculationMixin):
    ...
    supplier_reference = models.CharField(
        _("Referencia Proveedor"), max_length=50, blank=True, help_text="Ej: Nro Factura Proveedor"
    )
    supplier = models.ForeignKey(
        "contacts.Contact", on_delete=models.PROTECT, related_name="purchase_orders"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        is_paying = self.status == self.Status.PAID and (
            is_new or self._initial_status != self.Status.PAID
        )
        ...
        if sub_id_match:
            try:
                ...
                if sub:
                    ...
                    sub.save()
                    print(
                        f"Subscription {sub.id} advanced from {old_date} to {new_date} due to PO {self.number} payment."
                    )
...
class PurchaseLine(models.Model):
    ...
    quantity = models.DecimalField(
        _("Cantidad"), max_digits=10, decimal_places=2, validators=[MinValueValidator(0)]
    )
    ...
    unit_cost = models.DecimalField(
        _("Costo Unitario"), max_digits=12, decimal_places=0, validators=[MinValueValidator(0)]
    )
...
class PurchaseReceipt(TransactionalDocument, TotalsCalculationMixin):
    ...
    delivery_reference = models.CharField(
        _("Referencia de Despacho/Guía"), max_length=100, blank=True
    )
...
class PurchaseReceiptLine(models.Model):
    ...
    total_cost = models.DecimalField(
        _("Costo Total"), max_digits=12, decimal_places=0, editable=False
    )
