...
class SubscriptionViewSet(viewsets.ModelViewSet):
    ...
    queryset = (
        Subscription.objects.all().select_related("product", "supplier").order_by("-created_at")
    )
    ...
    def stats(self, request):
        ...
        total_monthly_cost = (
            Subscription.objects.filter(status=Subscription.Status.ACTIVE).aggregate(
                total=Sum("amount")
            )["total"]
            or 0
        )
            ...
            notes_data.append(
                {
                    ...
                    "purchase_order_number": note.purchase_order.number
                    if note.purchase_order
                    else None,
                }
            )
