...
from .models import Product, StockMove, Subscription
...
def product_post_save(sender, instance, created, **kwargs):
    ...
    cost_changed = (created and instance.cost_price > 0) or (
        not created and old_cost != instance.cost_price
    )
    sale_changed = (created and instance.sale_price > 0) or (
        not created and old_sale != instance.sale_price
    )
...
def product_subscription_sync(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if not instance.subscription_supplier or (
            not instance.is_variable_amount and not instance.subscription_amount
        ):
        ...
        subscription.next_payment_date = SubscriptionService.calculate_next_payment_date(
            subscription, from_date=start_date
        )
        ...
        active_subs = Subscription.objects.filter(
            product=instance, status=Subscription.Status.ACTIVE
        )
        ...
        if active_subs.exists():
            for sub in active_subs:
                ...
                sub.recurrence_period = (
                    instance.recurrence_period or Product.RecurrencePeriod.MONTHLY
                )
            ...
            logger.info(
                f"Synchronized {active_subs.count()} active subscriptions for product {instance.id}"
            )
