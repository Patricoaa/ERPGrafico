...
def test_create_note_block_service_return(basic_setup):
    ...
    with pytest.raises(
        ValidationError, match="No se pueden registrar devoluciones físicas para servicios"
    ):
        SalesService.create_note(
            order=order,
            note_type=Invoice.DTEType.NOTA_CREDITO,
...
def test_block_debit_note_for_non_storable_manufacturable(basic_setup):
    ...
    with pytest.raises(
        ValidationError, match="No se permite crear Nota de Débito para el producto fabricado"
    ):
        SalesService.create_note(
            order=order,
