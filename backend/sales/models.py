...
from core.models import TimeStampedModel, TransactionalDocument, User
...
class SaleOrder(TransactionalDocument, TotalsCalculationMixin):
    ...
    customer = models.ForeignKey(
        "contacts.Contact", on_delete=models.PROTECT, related_name="sale_orders"
    )
    ...
    channel = models.CharField(
        _("Canal"), max_length=20, choices=Channel.choices, default=Channel.SYSTEM
    )
    ...
    total_discount_amount = models.DecimalField(
        _("Descuento Total"), max_digits=12, decimal_places=0, default=0
    )
...
class SaleLine(models.Model):
    ...
    first_name = models.CharField(
        _("Nombre"), max_length=100, blank=True
    )  # Not relevant for context but matching context lines
    ...
    quantity = models.DecimalField(
        _("Cantidad"), max_digits=10, decimal_places=2, default=1, validators=[MinValueValidator(0)]
    )
    ...
    unit_price = models.DecimalField(
        _("Precio Unitario"), max_digits=12, decimal_places=0, validators=[MinValueValidator(0)]
    )
    unit_price_gross = models.DecimalField(
        _("Precio Unitario Bruto"),
        max_digits=12,
        decimal_places=0,
        null=True,
        blank=True,
        ...
        if self.product and self.uom and self.product.uom:
            if self.product.uom.category_id != self.uom.category_id:
                ...
                raise ValidationError(
                    f"La unidad {self.uom.name} no pertenece a la categoría {self.product.uom.category.name}"
                )
...
class SalesSettings(TimeStampedModel):
    ...
    restrict_stock_sales = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Si está activo, impide vender productos almacenables si no hay stock suficiente."
        ),
    )
    pos_enable_line_discounts = models.BooleanField(
        _("POS: Habilitar Descuentos por Línea"), default=True
    )
    pos_enable_total_discounts = models.BooleanField(
        _("POS: Habilitar Descuento Total"), default=True
    )
    ...
    pos_line_discount_group = models.CharField(
        _("Grupo con permiso para Descuento por Línea"), max_length=100, blank=True
    )
    ...
    pos_global_discount_group = models.CharField(
        _("Grupo con permiso para Descuento Global"), max_length=100, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        from core.cache import CACHE_KEY_SALES_SETTINGS, invalidate_singleton

        invalidate_singleton(CACHE_KEY_SALES_SETTINGS)

    @classmethod
    def get_solo(cls):
        from core.cache import CACHE_KEY_SALES_SETTINGS, cached_singleton
...
class SaleDelivery(TransactionalDocument, TotalsCalculationMixin):
    ...
    total_cost = models.DecimalField(
        _("Costo Total (COGS)"), max_digits=12, decimal_places=0, default=0
    )
...
class SaleDeliveryLine(models.Model):
    ...
    product = models.ForeignKey(
        "inventory.Product", on_delete=models.PROTECT, related_name="delivery_lines"
    )
    uom = models.ForeignKey(
        "inventory.UoM",
        on_delete=models.PROTECT,
    ...
    subtotal = models.DecimalField(
        _("Subtotal"), max_digits=12, decimal_places=0, editable=False, default=0
    )
    total_cost = models.DecimalField(
        _("Costo Total"), max_digits=12, decimal_places=0, editable=False, default=0
    )
...
class SaleReturn(TransactionalDocument, TotalsCalculationMixin):
    ...
    total_cost = models.DecimalField(
        _("Costo Total (COGS Reverso)"), max_digits=12, decimal_places=0, default=0
    )
...
class SaleReturnLine(models.Model):
    ...
    quantity = models.DecimalField(
        _("Cantidad"), max_digits=10, decimal_places=2, default=0, validators=[MinValueValidator(0)]
    )

    # Value reference
    unit_price = models.DecimalField(
        _("Precio Unitario"), max_digits=12, decimal_places=0, default=0
    )
    unit_price_gross = models.DecimalField(
        _("Precio Unitario Bruto"),
        max_digits=12,
...
class DraftCart(models.Model):
    ...
    name = models.CharField(
        _("Nombre"), max_length=255, blank=True, help_text=_("Nombre descriptivo del borrador")
    )

    notes = models.TextField(_("Notas"), blank=True)

    items = models.JSONField(
        _("Items del Carrito"), help_text=_("Estructura JSON con los items del carrito")
    )
    ...
    total_net = models.DecimalField(_("Total Neto"), max_digits=12, decimal_places=2, default=0)

    total_gross = models.DecimalField(_("Total Bruto"), max_digits=12, decimal_places=2, default=0)

    total_discount_amount = models.DecimalField(
        _("Descuento Total"), max_digits=12, decimal_places=2, default=0
    )
