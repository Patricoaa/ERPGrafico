...
class SalesSettingsViewSet(viewsets.ModelViewSet, AuditHistoryMixin):
    ...
    def current(self, request):
        ...
        sales_fields = {
            k: v for k, v in request.data.items() if k not in self.ACCOUNTING_SETTINGS_FIELDS
        }
        accounting_fields = {
            k: v for k, v in request.data.items() if k in self.ACCOUNTING_SETTINGS_FIELDS
        }
...
class SaleOrderViewSet(viewsets.ModelViewSet, AuditHistoryMixin):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if pos_pin:
            ...
            if not pin_user:
                return Response(
                    {"error": "PIN de seguridad incorrecto."}, status=status.HTTP_403_FORBIDDEN
                )
    ...
    def write_off(self, request, pk=None):
        ...
        if balance <= 0:
            return Response(
                {"error": "Esta orden no tiene saldo pendiente para castigar."}, status=400
            )

        settings = AccountingSettings.get_solo()
        if not settings or not settings.default_uncollectible_expense_account:
            return Response(
                {"error": "No hay una cuenta de gasto por incobrabilidad configurada."}, status=400
            )
        ...
        if not receivable_account:
            return Response(
                {"error": "No se encontró una cuenta por cobrar configurada."}, status=400
            )
