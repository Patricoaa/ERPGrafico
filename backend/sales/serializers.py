...
class SaleOrderSerializer(serializers.ModelSerializer):
    ...
    def get_command_center_status(self, obj):
        ...
        # Calculate stages completeness
        stages = []
        
        # Production Stage
        has_production = obj.work_orders.exists() or any(l.product.product_type == 'MANUFACTURABLE' for l in obj.lines.all())
        if has_production:
            # Check if all OTs are finished
            # Using the pre-fetched work_orders from the serializer might be safer but obj.work_orders.all() is cached if prefetched
            wos = obj.work_orders.all()
            production_complete = wos.exists() and all(wo.status == 'FINISHED' for wo in wos)
            stages.append(production_complete)

        # Logistics Stage
        # Logic adapted from frontend: total delivered / total ordered
        lines = obj.lines.all()
        if lines.exists():
            # Skip if only subscriptions (frontend logic)
            if not all(l.product.product_type == 'SUBSCRIPTION' for l in lines):
                total_ordered = sum(l.quantity for l in lines)
                total_delivered = sum(l.quantity_delivered for l in lines)
                
                # Avoid division by zero
                logistics_complete = total_ordered > 0 and total_delivered >= total_ordered
                stages.append(logistics_complete)

        # Billing Stage
        # Valid if has invoices and none are drafts
        ...
        # Treasury Stage
        # Paid and no pending transaction numbers
        payments = obj.payments.all()
        # Check pending transactions (frontend logic: outbound card/transfer or inbound transfer missing number)
        ...
        is_paid = (obj.status == 'PAID' or total_paid >= obj.total) and not has_pending_transactions
