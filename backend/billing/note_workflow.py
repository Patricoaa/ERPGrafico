...
class NoteWorkflow(models.Model):
    ...
    current_stage = models.CharField(
        _("Etapa Actual"), max_length=30, choices=Stage.choices, default=Stage.DRAFT
    )
    ...
    selected_items = models.JSONField(
        ...
        blank=True,
        help_text=_(
            "Lista de productos a devolver/ajustar: [{product_id, quantity, reason, unit_price, tax_amount}]"
        ),
    )
    ...
    reason = models.TextField(
        _("Motivo"), blank=True, help_text=_("Razón de la creación de esta nota")
    )
    ...
    def __str__(self):
        return (
            f"Workflow {self.id} - {self.invoice.display_id} ({self.get_current_stage_display()})"
        )
        ...
        if self.corrected_invoice_id:
            if self.corrected_invoice.status not in ["POSTED", "PAID"]:
                raise ValidationError(
                    {
                        "corrected_invoice": _(
                            "Solo se pueden crear NC/ND desde facturas publicadas o pagadas."
                        )
                    }
                )
    ...
    def can_advance(self):
        ...
        stage_requirements = {
            ...
            self.Stage.INVOICE_SELECTED: lambda: (
                self.selected_items is not None and len(self.selected_items) > 0
            ),
            self.Stage.ITEMS_SELECTED: lambda: (
                not self.requires_logistics or self.logistics_data is not None
            ),
