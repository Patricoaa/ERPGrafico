...
from inventory.models import Product, ProductCategory, StockMove, UoM, UoMCategory, Warehouse
...
def setup_test_data():
    ...
    acc_inventory, _ = Account.objects.get_or_create(
        code="1.2.01", defaults={"name": "Inventario", "account_type": "ASSET"}
    )
    ...
    customer, _ = Contact.objects.get_or_create(
        name="Test Customer NC", defaults={"tax_id": "11111111-1"}
    )
...
def test_complete_workflow():
    ...
    try:
        ...
        print(
            f"   Workflow created: ID={workflow.id}, Stage={workflow.get_current_stage_display()}"
        )
        print(
            f"   Note invoice: {workflow.invoice.display_id} (Status: {workflow.invoice.get_status_display()})"
        )
        ...
        workflow = NoteCheckoutService.register_document(
            workflow_id=workflow.id, document_number="NC-MANUAL-001", is_pending=False
        )
        print(f"   Document registered: {workflow.invoice.display_id}")
        print(f"   Status: {workflow.invoice.get_status_display()}")
        print(
            f"   Journal entry: ID={workflow.invoice.journal_entry.id if workflow.invoice.journal_entry else 'None'}"
        )
