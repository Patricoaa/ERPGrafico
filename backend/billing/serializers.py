                ...
                if hasattr(obj, "workflow") and obj.workflow and obj.workflow.selected_items:
                    ...
                    if is_credit:
                        ...
                        if is_sale:
                            ...
                            qs = SaleReturnLine.objects.filter(
                                return_doc__credit_note=obj, return_doc__status="CONFIRMED"
                            )
                            for line in qs:
                                processed_map[line.product_id] = processed_map.get(
                                    line.product_id, 0
                                ) + float(line.quantity)
                        else:
                        ...
                        if is_sale:
                            ...
                            qs = SaleDeliveryLine.objects.filter(
                                delivery__related_note=obj, delivery__status="CONFIRMED"
                            )
                            for line in qs:
                                processed_map[line.product_id] = processed_map.get(
                                    line.product_id, 0
                            ...
                            qs = PurchaseReceiptLine.objects.filter(
                                receipt__related_note=obj, receipt__status="CONFIRMED"
                            )
                            for line in qs:
                                processed_map[line.product_id] = processed_map.get(
                                    line.product_id, 0
