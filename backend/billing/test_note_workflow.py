...
def comprehensive_setup(db):
    ...
    acc_receivable = Account.objects.create(
        code="1.1.01", name="Cuentas por Cobrar", account_type=AccountType.ASSET
    )
    ...
    acc_inventory = Account.objects.create(
        code="1.2.01", name="Inventario", account_type=AccountType.ASSET
    )
    acc_tax_payable = Account.objects.create(
        code="2.1.02", name="IVA por Pagar", account_type=AccountType.LIABILITY
    )
...
def posted_sale_invoice(comprehensive_setup):
    ...
    order = SaleOrder.objects.create(
        customer=setup["customer"], status=SaleOrder.Status.CONFIRMED, payment_method="CREDIT"
    )
    ...
    return {"invoice": invoice, "order": order, **comprehensive_setup}
...
def test_init_workflow_valid_origin_statuses(comprehensive_setup):
    ...
    with pytest.raises(
        ValidationError, match="Solo se pueden crear NC/ND desde facturas publicadas o pagadas"
    ):
    ...
    workflow = NoteCheckoutService.init_note_workflow(
        corrected_invoice_id=paid_invoice.id, note_type=Invoice.DTEType.NOTA_CREDITO, reason="Test"
    )
...
def test_select_items_excludes_advanced_manufacturable(comprehensive_setup):
    ...
    order = SaleOrder.objects.create(customer=setup["customer"], status=SaleOrder.Status.CONFIRMED)
    ...
    # Create workflow
    workflow = NoteCheckoutService.init_note_workflow(
        corrected_invoice_id=invoice.id, note_type=Invoice.DTEType.NOTA_CREDITO, reason="Test"
    )
...
def test_complete_workflow_end_to_end(posted_sale_invoice):
    ...
    workflow = NoteCheckoutService.complete_workflow(
        workflow_id=workflow.id, payment_data={"method": "CREDIT", "apply_credit": True}
    )
...
def test_cancel_workflow(posted_sale_invoice):
    ...
    workflow = NoteCheckoutService.cancel_workflow(
        workflow_id=workflow.id, reason="Cliente retiró solicitud"
    )
