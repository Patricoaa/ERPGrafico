...
class NoteCheckoutService:
    ...
    def process_logistics_from_invoice(
        ...
        if is_sale:
            ...
            for line in lines:
                price_map[line.product_id] = (line.unit_price, line.id)
        else:
            lines = invoice.note_purchase_lines.all()
            for line in lines:
                price_map[line.product_id] = (line.unit_cost, line.id)
        ...
        for ld in line_data:
            ...
            if mapping:
                ...
                unit_val = (
                    product.cost_price if not is_sale else product.cost_price * Decimal("1.2")
                )
                line_id = None
    ...
    def init_note_workflow(
        corrected_invoice_id: int, note_type: str, reason: str = "", created_by=None
    ) -> NoteWorkflow:
        """
        ...
        if corrected_invoice.dte_type in [
            Invoice.DTEType.NOTA_CREDITO,
            Invoice.DTEType.NOTA_DEBITO,
        ]:
            raise ValidationError(
                "No se puede crear una nota desde otra nota. Seleccione la factura original."
            )
    ...
    @staticmethod
    @transaction.atomic
    def select_items(workflow_id: int, selected_items: List[Dict]) -> NoteWorkflow:
        """
        ...
        """
        workflow = NoteWorkflow.objects.select_related("invoice", "corrected_invoice").get(
            id=workflow_id
        )
        ...
        for item in selected_items:
            ...
            if tax_amount == 0 and not is_exempt:
                ...
                tax_amount = (unit_price * (tax_rate / 100)).quantize(
    ...
    def process_logistics(
        ...
        if logistics_doc:
            doc_type_label = (
                "Devolución" if is_credit_note else ("Despacho" if is_sale else "Recepción")
            )
    ...
    def skip_logistics(workflow_id: int) -> NoteWorkflow:
        ...
        if workflow.requires_logistics:
            raise ValidationError("No se puede saltar logística cuando hay productos stockeables.")
    ...
    def register_document(
        ...
        if (
            workflow.current_stage == NoteWorkflow.Stage.ITEMS_SELECTED
            and not workflow.requires_logistics
        ):
            workflow.current_stage = NoteWorkflow.Stage.LOGISTICS_COMPLETED
            workflow.save()
        ...
        if workflow.is_credit_note and not is_pending and not document_attachment:
            raise ValidationError(
                "El archivo PDF/XML es obligatorio para emitir la nota de crédito."
            )
    ...
    def _trigger_production_for_debit_note(workflow: NoteWorkflow):
        ...
        for item in workflow.selected_items:
            ...
            if product.requires_advanced_manufacturing:
                ...
                print(
                    f"DEBUG: Skipping EXPRESS product {product.internal_code} - OT will be created at dispatch"
                )
    ...
    def _create_accounting_entry(
        ...
        moved_quantities: Dict[int, Decimal] = None,
    ) -> JournalEntry:
        """
        ...
        for item in workflow.selected_items:
            ...
            if product.product_type == Product.Type.SERVICE:
                ...
                if is_sale:
                    ...
                    product_account = (
                        settings.stock_input_account or settings.default_expense_account
                    )
            else:
            ...
            if not product_account:
                ...
                raise ValidationError(
                    f"No se encontró cuenta de {account_req} para '{product.name}'."
                )
        ...
        if invoice.total_tax > 0 and not is_purchase_boleta:
            ...
            if not tax_account:
                ...
                raise ValidationError(
                    f"No se encontró cuenta de {tax_type} por defecto en la configuración contable."
                )
    ...
    def process_payment(workflow_id: int, payment_data: Dict) -> NoteWorkflow:
        ...
        if workflow.current_stage != NoteWorkflow.Stage.PAYMENT_PENDING:
            raise ValidationError(
                f"No se puede procesar pago en etapa {workflow.get_current_stage_display()}"
            )
    ...
    def _process_full_checkout_internal(
                        ...
                        items_to_move.append(
                            {
                                ...
                                "unit_cost": Decimal(
                                    str(match.get("unit_price", 0))
                                ),  # Supplemental items use it for purchase cost
                            }
                        )
        ...
        entry = NoteCheckoutService._create_accounting_entry(
            workflow, AccountingSettings.get_solo(), moved_quantities=moved_quantities
        )
