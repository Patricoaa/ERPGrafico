...
class InventoryCostingTestCase(TestCase):
    def setUp(self):
        ...
        self.inventory_account = Account.objects.create(
            code="1100", name="Inventario", account_type=AccountType.ASSET
        )
        self.payable_account = Account.objects.create(
            code="2100", name="Cuentas por Pagar", account_type=AccountType.LIABILITY
        )
        self.stock_input_account = Account.objects.create(
            code="2150", name="Facturas por Recibir", account_type=AccountType.LIABILITY
        )
        self.tax_receivable_account = Account.objects.create(
            code="1200", name="IVA Crédito Fiscal", account_type=AccountType.ASSET
        )
        ...
        self.category = ProductCategory.objects.create(
            name="Test Category", asset_account=self.inventory_account
        )

        # Create product
        self.product = Product.objects.create(
            code="TEST001", name="Test Product", category=self.category, cost_price=Decimal("0")
        )
    ...
    def test_boleta_capitalizes_vat(self):
        ...
        PurchasingService.receive_order(
            order=po, warehouse=self.warehouse, receipt_date=timezone.now().date()
        )
        ...
        BillingService.create_purchase_bill(
            order=po, supplier_invoice_number="BOL-001", dte_type="BOLETA", status="POSTED"
        )
    ...
    def test_boleta_then_receive_preserves_vat(self):
        ...
        BillingService.create_purchase_bill(
            order=po, supplier_invoice_number="BOL-002", dte_type="BOLETA", status="POSTED"
        )
    ...
    def test_draft_factura_then_confirm_reverts_vat(self):
        ...
        invoice = BillingService.create_purchase_bill(
            order=po, supplier_invoice_number="", dte_type="FACTURA", status="DRAFT"
        )
