...
class Transition(models.Model):
    ...
    transition = models.CharField(
        _("Transición"), max_length=20, choices=Type.choices, db_index=True
    )
...
class Task(models.Model):
    ...
    status = models.CharField(
        _("Estado"), max_length=20, choices=Status.choices, default=Status.PENDING, db_index=True
    )
    priority = models.CharField(
        _("Prioridad"), max_length=20, choices=Priority.choices, default=Priority.MEDIUM
    )
...
class Notification(models.Model):
    ...
    link = models.CharField(
        _("Enlace"), max_length=500, blank=True, help_text="URL relativa para redirigir al usuario"
    )
    data = models.JSONField(default=dict, blank=True)
    notification_type = models.CharField(
        _("Tipo de Evento"), max_length=100, blank=True, db_index=True
    )
...
class WorkflowSettings(TimeStampedModel):
    ...
    low_margin_threshold_percent = models.DecimalField(
        ...
        help_text=_(
            "Si el margen de un producto baja de este porcentaje, se envía una notificación (0 para desactivar)."
        ),
    )
    ...
    def save(self, *args, **kwargs):
        ...
        from core.cache import CACHE_KEY_WORKFLOW_SETTINGS, invalidate_singleton

        invalidate_singleton(CACHE_KEY_WORKFLOW_SETTINGS)

    @classmethod
    def get_settings(cls):
        from core.cache import CACHE_KEY_WORKFLOW_SETTINGS, cached_singleton
...
class NotificationRule(models.Model):
    ...
    def __str__(self):
        destinatarios = []
        if self.notify_creator:
            destinatarios.append("Creador")
        if self.assigned_user:
            destinatarios.append(str(self.assigned_user))
        if self.assigned_group:
            destinatarios.append(f"Grupo:{self.assigned_group.name}")
