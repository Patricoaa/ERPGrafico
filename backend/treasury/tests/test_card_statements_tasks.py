...
from treasury.models import Bank, CreditCardStatement, TreasuryAccount
...
def test_mark_overdue_credits_open_past_due_as_overdue(env):
    ...
    future = _open(env, year=ny, month=nm, due=today + timedelta(days=3), billed=Decimal("50000"))
...
def test_mark_overdue_does_not_touch_paid_or_canceled(env):
    ...
    ny, nm = _next_month(today)
    canceled = _open(
        env, year=ny, month=nm, due=today - timedelta(days=20), billed=Decimal("100000")
    )
