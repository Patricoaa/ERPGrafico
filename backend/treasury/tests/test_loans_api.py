...
def test_pay_installment_action_closes_loan(
    ...
    create = auth_client.post(
        "/api/treasury/loans/",
        _make_payload(
            bank, checking_account, liability_accounting, term_months=2, principal="1000000.00"
        ),
        format="json",
    )
    loan_id = create.json()["id"]
    ...
    installments = list(LoanInstallment.objects.filter(loan_id=loan_id).order_by("number"))
    # Pagar la primera
    r1 = auth_client.post(
...
def test_prepay_action_marks_all_canceled(
    ...
    assert (
        BankLoan.objects.get(pk=loan_id)
        .installments.filter(status=LoanInstallment.Status.CANCELED)
        .count()
        == 6
    )
...
def test_disburse_with_commission_expense_account_override(
    ...
    commission_acc = Account.objects.create(
        name="Comisión Apertura (override)",
        code="5.2.01.030",
        account_type=AccountType.EXPENSE,
    )
...
def test_bank_overview_aggregates_active_loan_debt(
    ...
    expected = (
        ...
        .exclude(status__in=[LoanInstallment.Status.PAID, LoanInstallment.Status.CANCELED])
