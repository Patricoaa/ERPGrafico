...
def base(db):
    ...
    interest_acc = Account.objects.create(
        name="Gasto Interés Bancario",
        code="5.2.01.010",
        account_type=AccountType.EXPENSE,
    )
    insurance_acc = Account.objects.create(
        name="Gasto Seguro Bancario",
        code="5.2.01.020",
        account_type=AccountType.EXPENSE,
    )
...
def test_disburse_with_fees_nets_cash_and_books_expense(base):
    ...
    bank_debit = JournalItem.objects.filter(entry=entry, account=base["bank_ta"].account).aggregate(
        s=Sum("debit")
    )["s"]
    liab_credit = JournalItem.objects.filter(
        entry=entry, account=base["liab_ta"].account
    ).aggregate(s=Sum("credit"))["s"]
    comm_debit = JournalItem.objects.filter(entry=entry, account=commission_acc).aggregate(
        s=Sum("debit")
    )["s"]
    stamp_debit = JournalItem.objects.filter(entry=entry, account=stamp_acc).aggregate(
        s=Sum("debit")
    )["s"]
