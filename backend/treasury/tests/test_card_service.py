...
def env(db):
    ...
    bank_ta = TreasuryAccount.objects.create(
        name='Cta Cte Banco', account=bank_acc,
        account_type=TreasuryAccount.Type.CHECKING,
        bank=bank, account_number='111',
    )
    ...
    return {
        ...
        'interest_exp': interest_exp, 'fees_exp': fees_exp,
    }
...
def test_apply_charges_only_interest(env):
    ...
    CardService.apply_charges(
        stmt, interest_expense_account=env['interest_exp'],
        created_by=env['user'],
    )
...
def test_pay_statement_transfers_bank_to_card(env):
    ...
    customer_acc = Account.objects.create(
    ...
    invoice = Invoice.objects.create(
        contact=customer, date=date(2026, 6, 1), number='FAC-TEST-1',
    )
    TreasuryService.create_movement(
    ...
    CardService.pay_statement(
        ...
        date=date(2026, 7, 25), created_by=env['user'],
    )
...
def test_pay_statement_idempotent(env):
    ...
    invoice = Invoice.objects.create(
        contact=customer, date=date(2026, 6, 1), number='FAC-IDEMP-1',
    )
    TreasuryService.create_movement(
...
def test_pay_statement_with_interest_and_fees(env):
    ...
    invoice = Invoice.objects.create(
        contact=customer, date=date(2026, 6, 1), number='FAC-CHG-1',
    )
    TreasuryService.create_movement(
        amount=Decimal('300000'),
        movement_type=TreasuryMovement.Type.INBOUND,
...
def test_cancel_statement_paid_raises(env):
    ...
    invoice = Invoice.objects.create(
        contact=customer, date=date(2026, 6, 1), number='FAC-CANC-1',
    )
    TreasuryService.create_movement(
        amount=Decimal('5000'),
        movement_type=TreasuryMovement.Type.INBOUND,
