...
def fixture(db):
    ...
    bank_ta = TreasuryAccount.objects.create(
        ...
        bank=bank, account_number='123456',
    )
...
def test_purchase_with_credit_card_increases_liability(fixture):
    ...
    liability_line = next(
        (it for it in items if it.account_id == fixture['card_acc'].id), None
    )
    ...
    payable_line = next(
        (it for it in items if it.account_id == fixture['payable_acc'].id), None
    )
