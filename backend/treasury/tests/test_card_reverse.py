...
def _env_setup():
    ...
    from accounting.models import Account, AccountingSettings, JournalEntry, JournalItem
    from treasury.models import TreasuryAccount, Bank
    ...
    bank_acc = Account.objects.create(
        code='1.1.01.10', name='Banco Cta Cte',
        account_type=AccountType.ASSET,
    )
    card_acc = Account.objects.create(
        code='2.1.09.01', name='Visa Pasivo',
        account_type=AccountType.LIABILITY,
    )
    bank_ta = TreasuryAccount.objects.create(
        bank=bank, name='Cuenta Corriente',
        account=bank_acc,
    ...
    interest_exp = Account.objects.create(
        code='6.1.01.01', name='Intereses Pagados',
        account_type=AccountType.EXPENSE,
    )
    fees_exp = Account.objects.create(
        code='6.1.01.02', name='Comisiones Bancarias',
        account_type=AccountType.EXPENSE,
    )
...
def test_reverse_statement_with_charges_only():
    ...
    CardService.apply_charges(
        ...
        fees_expense_account=env['fees_exp'], created_by=env['user'],
    )
