...
from treasury.models import BankStatement, TreasuryAccount
...
def account(db):
    accounting_account = Account.objects.create(
        name="Caja Test", code="1.1.01.999", account_type="ASSET"
    )
    return TreasuryAccount.objects.create(
        name="Test Account", account_type="CASH", currency="CLP", account=accounting_account
    )
...
class TestReconciliationOverlap:
    def test_overlapping_with_confirmed(self, account, user):
        ...
        assert any(
            "se solapa con una cartola confirmada existente" in err for err in result["errors"]
        )
    ...
    def test_overlapping_with_draft(self, account, user):
        ...
        assert any(
            "se solapa con una cartola en borrador" in warn["message"]
