...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        checking_no_number = TreasuryAccount.objects.filter(
            account_type="CHECKING", account_number__isnull=True
        )
        ...
        checking_no_bank = TreasuryAccount.objects.filter(
            account_type="CHECKING", bank__isnull=True
        )
        ...
        cards_no_bank = TreasuryAccount.objects.filter(
            account_type=TreasuryAccount.Type.CREDIT_CARD, bank__isnull=True
        )
        ...
        cash_with_bank = TreasuryAccount.objects.filter(account_type="CASH", bank__isnull=False)
        ...
        for method in PaymentMethod.objects.select_related("treasury_account").all():
            try:
                ...
                issues.append(
                    f"[ERROR] Metodo '{method.name}' incompatible con cuenta '{method.treasury_account.name}'"
                )
