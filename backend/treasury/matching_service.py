...
class MatchingService:
    ...
    @staticmethod
    def suggest_matches(statement_line_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def _suggest_batches(line: BankStatementLine, limit: int) -> List[Dict[str, Any]]:
        ...
        for b in batches[:limit]:
            ...
            if (
                b.terminal_reference
                and line.reference
                and b.terminal_reference.upper() in line.reference.upper()
            ):
                score += 10
    ...
    @staticmethod
    def suggest_lines_for_payment(payment_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        filters = Q(statement__treasury_account=account, reconciliation_status="UNRECONCILED")
        ...
        lines_query = BankStatementLine.objects.filter(filters & candidate_filters).select_related(
            "statement"
        )
    ...
    def _calculate_match_score(
        ...
        total_weight = (
            settings.amount_weight
            + settings.date_weight
            + settings.reference_weight
            + settings.contact_weight
        )
        if total_weight == 0:
    ...
    def create_match_group(
        ...
        if (
            len(lines) != len(line_ids)
            or len(payments) != len(movement_ids)
            or (batch_ids and len(batches) != len(batch_ids))
        ):
            raise ValueError("Algunas líneas, pagos o lotes no existen")

        # Validaciones de estado y consistencia
        is_all_abonos = all(line.credit > 0 for line in lines)
        is_all_cargos = all(line.debit > 0 for line in lines)

        if not (is_all_abonos or is_all_cargos):
            raise ValueError(
                "No se pueden mezclar Cargos y Abonos en un mismo grupo de conciliación."
            )
        ...
        if not (is_all_inbound or is_all_outbound):
            raise ValueError(
                "No se pueden mezclar Ingresos y Egresos en un mismo grupo de conciliación."
            )

        # Validación de sentido cruzado
        if is_all_abonos and not (
            is_all_inbound or (batches and all(b.net_amount > 0 for b in batches))
        ):
            raise ValueError(
                "Los Abonos bancarios solo pueden conciliarse con Ingresos del sistema o Lotes de Terminal."
            )
        if is_all_cargos and not is_all_outbound:
            raise ValueError(
                "Los Cargos bancarios solo pueden conciliarse con Egresos del sistema."
            )

        for line in lines:
            if line.reconciliation_status == "RECONCILED":
                raise ValueError(f"Línea {line.line_number} ya reconciliada")
            if line.statement.status == "CONFIRMED":
                raise ValueError(f"Cartola {line.statement.display_id} está confirmada")
        ...
        group = ReconciliationMatch.objects.create(
            treasury_account=treasury_account, created_by=user, is_confirmed=False
        )

        # Link Lines
        total_lines_amount = Decimal(0)
        for line in lines:
            line.reconciliation_match = group
            line.reconciliation_status = "MATCHED"
            line.save()
            total_lines_amount += abs(line.credit - line.debit)
        ...
        if lines and total_lines_amount > 0:
            ...
            for i, line in enumerate(lines):
                ...
                if i == len(lines) - 1:
                    line_diff = remaining_diff
                else:
                    line_abs = abs(line.credit - line.debit)
                    proportion = line_abs / total_lines_amount
                    line_diff = (difference * proportion).quantize(Decimal("0.01"))
                    remaining_diff -= line_diff

                line.difference_amount = line_diff
                if difference_reason:
                    line.difference_reason = difference_reason
                line.save()

                if line_diff != 0:
                    distribution_log.append(f"L{line.line_number}: {line_diff}")
    ...
    @staticmethod
    @transaction.atomic
    def manual_match(statement_line_id: int, payment_id: int, user) -> BankStatementLine:
        """
    ...
    @staticmethod
    @transaction.atomic
    def confirm_match(statement_line_id: int, user) -> BankStatementLine:
        """
        ...
        BankStatementLine.objects.bulk_update(
            lines_in_group, ["reconciliation_status", "reconciled_at", "reconciled_by"]
        )
    ...
    def unmatch(statement_line_id: int) -> BankStatementLine:
        ...
        for line in lines_to_reset:
            line.reconciliation_match = None
            line.reconciliation_status = "UNRECONCILED"
            line.reconciled_at = None
            line.difference_amount = Decimal(0)
        BankStatementLine.objects.bulk_update(
            lines_to_reset,
            ["reconciliation_match", "reconciliation_status", "reconciled_at", "difference_amount"],
        )
    ...
    def auto_match_statement(
        statement_id: int, confidence_threshold: float = 90.0
    ) -> Dict[str, Any]:
        """
        ...
        if _HAS_RUST and len(unreconciled_lines) * len(all_candidates) > 100:
            ...
            for line in unreconciled_lines:
                line_candidates = [
                    ...
                    and MatchingService._payment_matches_account_sense(
                        p, account, line.credit > line.debit
                    )
                ]
