...
from .parsers.formats import get_available_formats, get_parser_config
...
class ReconciliationService:
    ...
    def import_statement(
        ...
        if (
            treasury_account.default_bank_format
            and treasury_account.default_bank_format != bank_format
        ):
    ...
    def dry_run_import(
        ...
        if BankStatement.objects.filter(file_hash=file_hash).exists():
            is_duplicate = True
            warnings.append(
                {"line": None, "message": "Este archivo ya ha sido importado anteriormente."}
            )
    ...
    def validate_statement(
        parsed_data: Dict[str, Any], treasury_account: TreasuryAccount
    ) -> Dict[str, Any]:
        """
        ...
        if lines:
            ...
            total_movements = sum(line["credit"] - line["debit"] for line in lines)
        ...
        if period_start and period_end:
            ...
            if overlapping.filter(status="CONFIRMED").exists():
                errors.append(
                    f"El rango de fechas ({period_start} a {period_end}) se solapa con una cartola confirmada existente."
                )
            ...
            previous_statement = (
                BankStatement.objects.filter(
                    treasury_account=treasury_account, period_end__lt=period_start
                )
                .order_by("-period_end")
                .first()
            )
        ...
        if lines:
            ...
            for i, line in enumerate(lines):
                ...
                if not diff.is_nan() and diff > Decimal(
                    "0.05"
                ):  # Un poco más permisivo por acumulacion
    ...
    def get_statement_summary(statement_id: int) -> Dict[str, Any]:
        ...
        reconciled_count = lines.filter(reconciliation_status="RECONCILED").count()

        matched_count = lines.filter(reconciliation_status="MATCHED").count()
    ...
    def generate_preview(file) -> Dict[str, Any]:
        ...
        try:
            if filename.endswith(".csv"):
                ...
                file_type = "excel"
                df = pd.read_excel(
                    file, nrows=10, header=None
                )  # Leer sin header para mostrar todo raw
            else:
