        ...
        try:
            ...
            for i, row in df_preview.iterrows():
                ...
                if "FECHA" in row_str and "DESCRIPCI" in row_str:
                    header_row = (
                        i + 1
                    )  # +1 porque el header es la siguiente fila en read_excel index
                    break
            ...
            first_row = df.iloc[0]
            first_bal = self.normalize_amount(
                first_row[mapped_cols["balance"]], decimal_separator="."
            )
            first_cred = self.normalize_amount(
                first_row[mapped_cols["credit"]], decimal_separator="."
            )
            first_deb = self.normalize_amount(
                first_row[mapped_cols["debit"]], decimal_separator="."
            )
            ...
            for index, row in df.iterrows():
                ...
                date_val = self.normalize_date(
                    row[mapped_cols["transaction_date"]], date_format="%d/%m/%Y"
                )
                if not date_val:
                    continue
            ...
            return {
                ...
                "lines": lines,
                "metadata": {"bank": "SANTANDER", "original_filename": file.name},
            }
