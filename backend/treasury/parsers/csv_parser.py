...
class GenericCSVParser(BaseParser):
    ...
    def parse(self, file) -> Dict[str, Any]:
        ...
        opening_balance = self._extract_balance_from_config(
            all_rows, balance_info, "opening_balance_row"
        )
        closing_balance = self._extract_balance_from_config(
            all_rows, balance_info, "closing_balance_row"
        )
        statement_date = self._extract_date_from_config(
            all_rows, balance_info, "statement_date_row"
        )
        ...
        if not lines:
            return {
                ...
                "lines": [],
                "metadata": {"total_lines": 0, "encoding": encoding, "format": "GENERIC_CSV"},
            }
        ...
        return {
            ...
            "lines": lines,
            "metadata": {"total_lines": len(lines), "encoding": encoding, "format": "GENERIC_CSV"},
        }

    def _parse_row(
        self, row: List[str], header: Optional[List[str]], line_number: int
    ...
    def _extract_balance_from_config(
        self, rows: List[List[str]], balance_info: Dict, key: str
    ) -> Optional[Decimal]:
        """
        ...
        for cell in row:
            try:
                return self.normalize_amount(cell, self.config.get("decimal_separator", "."))
            except ValueError:
                continue

        return None

    def _extract_date_from_config(
        self, rows: List[List[str]], balance_info: Dict, key: str
    ) -> Optional[date]:
        """
        ...
        for cell in row:
            parsed_date = self.normalize_date(cell, self.config.get("date_format", "%d-%m-%Y"))
            if parsed_date:
                return parsed_date
