...
class DifferenceService:
    ...
    def create_difference_adjustment(
        line: BankStatementLine, difference_type: str, user, notes: str = "", accounting_date=None
    ) -> JournalEntry:
        """
        ...
        if not settings:
            raise ValueError(
                "No existe configuración contable global. Por favor configure los ajustes de contabilidad."
            )
        ...
        if not difference_account:
            ...
            raise ValueError(
                f"No se ha configurado la cuenta contable para '{label}'. Revise la configuración contable."
            )
    ...
    def get_difference_summary(statement_id: int) -> Dict:
        ...
        lines_with_diff = statement.lines.filter(reconciliation_status="RECONCILED").exclude(
            difference_amount=0
        )
        total_positive = sum(
            line.difference_amount for line in lines_with_diff if line.difference_amount > 0
        )
        total_negative = sum(
            abs(line.difference_amount) for line in lines_with_diff if line.difference_amount < 0
        )
        by_type = {}
