...
class GlobalHRSettings(TimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        ...
        from core.cache import CACHE_KEY_HR_SETTINGS, invalidate_singleton

        invalidate_singleton(CACHE_KEY_HR_SETTINGS)

    @classmethod
    def get_solo(cls):
        from core.cache import CACHE_KEY_HR_SETTINGS, cached_singleton
...
class PayrollConcept(models.Model):
    ...
    class Category(models.TextChoices):
        ...
        DESCUENTO_LEGAL_TRABAJADOR = (
            "DESCUENTO_LEGAL_TRABAJADOR",
            _("Descuento Legal (Cargo Trabajador)"),
        )
    ...
    formula_type = models.CharField(
        _("Tipo de Cálculo"), max_length=20, choices=FormulaType.choices, default=FormulaType.FIXED
    )
    formula = models.TextField(
        _("Fórmula / Expresión"), blank=True, help_text=_("Ej: IMPONIBLE * 0.07 o BASE + 50000")
    )
    default_amount = models.DecimalField(
        _("Monto por defecto"), max_digits=14, decimal_places=4, default=Decimal("0")
    )

    is_system = models.BooleanField(
        default=False, help_text=_("Conceptos generados por el sistema para cálculos legales.")
    )
...
class EmployeeConceptAmount(models.Model):
    ...
    employee = models.ForeignKey(
        "Employee", on_delete=models.CASCADE, related_name="concept_amounts"
    )
    concept = models.ForeignKey(
        PayrollConcept, on_delete=models.CASCADE, related_name="employee_amounts"
    )
    amount = models.DecimalField(
        _("Monto Mensual"), max_digits=14, decimal_places=4, validators=[MinValueValidator(0)]
    )
...
class Employee(models.Model):
    ...
    status = models.CharField(
        _("Estado"), max_length=10, choices=Status.choices, default=Status.ACTIVE
    )
    contract_type = models.CharField(
        _("Tipo de Contrato"),
        max_length=20,
        choices=ContractType.choices,
        default=ContractType.INDEFINIDO,
    )
    ...
    afp = models.ForeignKey(
        AFP, on_delete=models.SET_NULL, null=True, related_name="employees", verbose_name=_("AFP")
    )
    salud_type = models.CharField(
        _("Sistema Salud"), max_length=10, choices=SaludType.choices, default=SaludType.FONASA
    )
    isapre_amount_uf = models.DecimalField(
        _("Monto Pactado Isapre (UF)"),
        max_digits=6,
        decimal_places=4,
        default=Decimal("0"),
    ...
    jornada_type = models.CharField(
        _("Tipo de Jornada"),
        max_length=20,
        choices=JornadaType.choices,
        default=JornadaType.ORDINARIA_22,
    )
    jornada_hours = models.DecimalField(
        _("Horas de Jornada"), max_digits=5, decimal_places=2, default=Decimal("44.0")
    )
    ...
    dias_pactados = models.IntegerField(
        _("Días Pactados"), default=30, validators=[MinValueValidator(1), MaxValueValidator(31)]
    )

    # Asignación Familiar
    asignacion_familiar = models.CharField(
        _("Asignación Familiar"),
        max_length=2,
        choices=AsignacionFamiliarTramo.choices,
        default=AsignacionFamiliarTramo.TRAMO_D,
    )
    cargas_familiares = models.IntegerField(
        _("Número de Cargas"), default=0, validators=[MinValueValidator(0)]
    )
...
class Payroll(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, related_name="payrolls", verbose_name=_("Empleado")
    )
    period_year = models.IntegerField(_("Año"))
    period_month = models.IntegerField(_("Mes"))
    status = models.CharField(
        _("Estado"), max_length=10, choices=Status.choices, default=Status.DRAFT
    )

    # Snapshots para auditoría
    base_salary = models.DecimalField(
        _("Sueldo Base (snapshot)"), max_digits=14, decimal_places=0, default=Decimal("0")
    )
    agreed_days = models.IntegerField(_("Días Pactados"), default=30)
    absent_days = models.DecimalField(
        _("Días Inasistencia"), max_digits=5, decimal_places=2, default=Decimal("0")
    )
    worked_days = models.DecimalField(
        _("Días Trabajados"), max_digits=5, decimal_places=2, default=Decimal("30")
    )

    # Totales Finales
    total_haberes = models.DecimalField(
        _("Total Haberes"), max_digits=14, decimal_places=0, default=Decimal("0")
    )
    total_descuentos = models.DecimalField(
        _("Total Descuentos"), max_digits=14, decimal_places=0, default=Decimal("0")
    )
    net_salary = models.DecimalField(
        _("Sueldo Líquido"), max_digits=14, decimal_places=0, default=Decimal("0")
    )
...
class PayrollItem(models.Model):
    ...
    concept = models.ForeignKey(
        PayrollConcept, on_delete=models.PROTECT, related_name="items", verbose_name=_("Concepto")
    )

    description = models.CharField(_("Descripción"), max_length=255, blank=True)
    amount = models.DecimalField(
        _("Monto"), max_digits=14, decimal_places=0, validators=[MinValueValidator(0)]
    )
...
class Absence(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.CASCADE, related_name="absences", verbose_name=_("Empleado")
    )
    absence_type = models.CharField(
        _("Tipo de Inasistencia"), max_length=20, choices=AbsenceType.choices
    )
...
class SalaryAdvance(models.Model):
    ...
    employee = models.ForeignKey(
        Employee, on_delete=models.PROTECT, related_name="advances", verbose_name=_("Empleado")
    )
    payroll = models.ForeignKey(
        ...
        verbose_name=_("Liquidación Asociada"),
        help_text=_("Si ya fue descontado de una liquidación, se vincula aquí."),
    )
    amount = models.DecimalField(
        _("Monto"), max_digits=14, decimal_places=0, validators=[MinValueValidator(Decimal("0.01"))]
    )
...
class PayrollPayment(models.Model):
    ...
    payroll = models.ForeignKey(
        "Payroll", on_delete=models.CASCADE, related_name="payments", verbose_name=_("Liquidación")
    )
    payment_type = models.CharField(
        _("Tipo"), max_length=10, choices=PaymentType.choices, default=PaymentType.SALARIO
    )
    amount = models.DecimalField(
        _("Monto Pagado"),
        max_digits=14,
